...
export default function NegocioDetalle({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function reenviar(pid: number, canal: string) { setBusy(true); try { const r = await vjson(`/negocios/propuestas/${pid}/reenviar?canal=${canal}`, { method: 'POST' }); if (canal === 'whatsapp' && r.whatsapp !== 'enviado' && r.wa_link) window.open(r.wa_link, '_blank'); setMsg({ ok: true, t: r.whatsapp === 'enviado' ? 'WhatsApp enviado automáticamente' : r.email === 'enviado' ? 'Correo reenviado' : r.email ? 'Correo: ' + r.email : 'Se abrió WhatsApp con el mensaje listo', wa: r.wa_link }); load() } catch (e: any) { setMsg({ ok: false, t: e.message }) } finally { setBusy(false) } }
