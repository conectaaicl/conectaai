  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">{/* eslint-disable-next-line @next/next/no-img-element */}<img src="/uploads/branding/terrablinds/logo.png" alt="TerraBlinds" className="w-14 h-14 rounded-2xl bg-white border border-[#DDE4E6] p-1 shrink-0 mt-1" /><div><Link href="/ventas/cortinas" className="text-xs text-[#7A8F98]">← Cortinas · TerraBlinds</Link><h1 className="text-2xl md:text-3xl font-extrabold">{l.nombre}</h1><p className="text-sm text-[#7A8F98]">{[l.direccion, l.comuna, l.tipo].filter(Boolean).join(' · ')}{l.origen !== 'terreno' ? ` · lead vía ${l.origen}` : ''}{l.notas ? ` · ${l.notas}` : ''}</p></div></div>
      ...
      <section className="bg-white border border-[#DDE4E6] rounded-2xl p-4 md:p-5">
        ...
        <div className="space-y-3">{esp.map((e, i) => (
          <div key={i} className="bg-[#F3F6F5] border border-[#DDE4E6] rounded-2xl p-3 grid grid-cols-2 md:grid-cols-6 gap-2 items-end">
            ...
            <label className="block md:col-span-2"><span className="text-[10px] font-bold uppercase text-[#7A8F98]">Producto</span>{cat?.productos.find((p: any) => p.key === e.producto)?.imagen && /* eslint-disable-next-line @next/next/no-img-element */ <img src={cat.productos.find((p: any) => p.key === e.producto).imagen} alt="" className="w-8 h-8 rounded-md object-cover inline-block ml-2 align-middle" />}<select className={inp} value={e.producto} onChange={ev => upd(i, 'producto', ev.target.value)}>{cat?.productos.map((p: any) => <option key={p.key} value={p.key}>{p.icon} {p.nombre} · {cat.precios[p.key] ? clp(cat.precios[p.key]) + (p.por_unidad ? '/u' : '/m²') : 'a cotizar'}</option>)}</select></label>
