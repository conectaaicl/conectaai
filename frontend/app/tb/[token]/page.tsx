...
export default function PropuestaTerraBlinds({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#F4EFE6] text-[#0B1F2A]">
      <div className="bg-[#0B1F2A] text-white"><div className="max-w-3xl mx-auto px-5 pt-10 pb-14">
        <p className="text-[11px] font-bold tracking-[.14em] uppercase text-[#C8B48A]">TerraBlinds · Propuesta de proyecto</p>
      ...
      <div className="max-w-3xl mx-auto px-5 -mt-8 space-y-5 pb-16">
        ...
        <section className="bg-white rounded-3xl border border-[#DDE4E6] shadow-xl p-6">
          <p className="text-[11px] font-bold tracking-[.12em] uppercase text-[#8A7340]">Tres formas de hacerlo</p><h2 className="text-xl font-extrabold mb-4">Elige tu nivel</h2>
          <div className="grid md:grid-cols-3 gap-3">{['esencial', 'confort', 'premium'].map(k => { const on = nivel === k; const rec = d.nivel_sugerido === k; return (
            <button key={k} disabled={d.aceptada} onClick={() => setNivel(k)} className={`text-left rounded-2xl border-2 p-4 transition ${on ? 'border-[#0B1F2A] bg-[#0B1F2A] text-white' : 'border-[#DDE4E6] bg-white'}`}>
              <div className={`text-[10px] font-bold uppercase tracking-wide ${on ? 'text-[#C8B48A]' : 'text-[#7A8F98]'}`}>{n[k].nombre}{rec ? ' · recomendado' : ''}</div>
          ...
          {d.descuento_pct > 0 && <p className="text-xs text-[#0F766E] font-semibold mt-3">Incluye {d.descuento_pct}% de descuento por confirmar dentro de 15 días.</p>}
          {!d.aceptada && !ok && <div className="mt-5 grid md:grid-cols-3 gap-2">
            ...
            <button onClick={aceptar} disabled={busy || !nombre} className="md:col-span-3 bg-[#0F766E] disabled:opacity-50 text-white font-extrabold py-4 rounded-2xl text-base">{busy ? 'Enviando…' : `Aceptar nivel ${n[nivel]?.nombre} · ${clp(n[nivel]?.total)}`}</button>
        ...
        <section className="bg-white rounded-3xl border border-[#DDE4E6] p-6">
          <p className="text-[11px] font-bold tracking-[.12em] uppercase text-[#8A7340]">Tu proyecto</p><h2 className="text-xl font-extrabold mb-3">Espacio por espacio, tal como lo medimos</h2>
          <div className="divide-y divide-[#DDE4E6]">{d.espacios.map((f: any, i: number) => <div key={i} className="py-3 flex items-center gap-3 text-sm"><span className="text-xl">{d.productos[f.producto]?.icon || '🪟'}</span><div className="flex-1"><b>{f.ambiente}</b> · {f.producto_nombre}{f.motorizado ? ' · motorizada' : ''}<div className="text-xs text-[#7A8F98]">{f.ancho_cm}×{f.alto_cm} cm{f.cantidad > 1 ? ` ×${f.cantidad}` : ''} · {f.m2} m²{f.color ? ` · ${f.color}` : ''}</div></div><span className="text-xs text-[#7A8F98]">ref. {clp(f.subtotal)}</span></div>)}</div>
          <div className="grid sm:grid-cols-2 gap-3 mt-4">{Object.values(d.productos).map((p: any) => <div key={p.key} className="bg-[#F4EFE6] rounded-2xl p-4"><b>{p.icon} {p.nombre}</b><p className="text-xs text-[#35505C] mt-1">{p.desc}</p></div>)}</div>
        </section>
        ...
        <section className="bg-white rounded-3xl border border-[#DDE4E6] p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-2"><a href={d.pdf_url} className="bg-white border border-[#DDE4E6] font-bold px-4 py-3 rounded-xl text-sm">Descargar PDF</a><a href={wa} target="_blank" rel="noreferrer" className="bg-[#0B1F2A] text-white font-bold px-4 py-3 rounded-xl text-sm">Tengo una duda</a><a href={d.web} target="_blank" rel="noreferrer" className="font-bold px-4 py-3 rounded-xl text-sm text-[#8A7340]">terrablinds.cl →</a></div>
