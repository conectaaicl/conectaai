...
export default function PortalRegistro() {
  ...
  const [nombreTenant, setNombreTenant] = useState('ConectaAI')
  const [unidadLabel, setUnidadLabel] = useState('Departamento')

  useEffect(() => {
    // Identidad real del tenant (nombre, gimnasio o no, casas o deptos) resuelta por el backend segun el dominio
    fetch('/api/portal/auth/info-publica')
      .then(r => r.ok ? r.json() : null)
      .then(info => {
        if (!info) return
        if (info.nombre) setNombreTenant(info.nombre)
        if (info.portal_label) setPortalLabel(info.portal_label)
        if (info.unidad_label) setUnidadLabel(info.unidad_label)
        setEsGym(!!info.es_gimnasio)
      })
      .catch(() => {
        if (typeof window !== 'undefined' && window.location.hostname.includes('gym.')) { setPortalLabel('Portal Socios'); setEsGym(true) }
      })
  }, [])
  ...
  const validate = () => {
    ...
    if (!deptoId && !esGym) errs.depto = 'Seleccione su ' + unidadLabel.toLowerCase()
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-600 to-indigo-900 flex flex-col items-center justify-center p-4 py-10">
      ...
      <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!esGym && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{unidadLabel} *</label>
            <select value={deptoId} onChange={e => setDeptoId(e.target.value)}
              className={inputClass(fe.depto)}>
              <option value="">Seleccione su {unidadLabel.toLowerCase()}</option>
              {deptos.map(d => (
