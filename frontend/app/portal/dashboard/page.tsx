  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
          ...
          <div>
            ...
            {(data.residente?.condominio || data.residente?.depto_numero) && (
              <p className="text-indigo-200 text-xs mt-0.5">
                ...
                {data.residente?.torre ? ' - ' + data.residente.torre : ''}
      ...
      <div className="max-w-lg mx-auto px-4 pb-4 space-y-4">
        ...
        <div className={`rounded-2xl border-2 p-5 ${sem.bg} ${sem.border}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className={`font-bold ${sem.text}`}>{data.semaforo_msg}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { label:'Estado de cuenta', href:'/portal/cuenta',     icon:'💰', bg:'bg-indigo-100 border-indigo-200' },
            { label:'Mis avisos',       href:'/portal/avisos',     icon:'📢', bg:'bg-amber-100 border-amber-200',   badge: data.avisos_no_leidos },
            { label:'Mensajes',         href:'/portal/mensajes',   icon:'💬', bg:'bg-sky-100 border-sky-200' },
            ...
            { label:'Mis vehículos',    href:'/portal/vehiculos',  icon:'🚗', bg:'bg-slate-100 border-slate-200' },
            ...((data.meses_vencidos || 0) >= 3 ? [{ label:'Convenio de pago', href:'/portal/convenio', icon:'🤝', bg:'bg-rose-100 border-rose-200' }] : []),
            ...
            <a key={item.label + item.href} href={item.href}
              className={`${item.bg} border-2 rounded-2xl p-4 flex flex-col items-start relative hover:-translate-y-0.5 hover:shadow-md transition-all`}>
