...
@router.get("/rubros")
def get_rubros(current_user: dict = Depends(get_current_user)):
    return RUBROS
...
def crear_proveedor(body: ProveedorCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    tenant_id = current_user["tenant_id"]
    _ensure_tables(db)
    ...
    ), {"tid": tenant_id, "nom": body.nombre, "rut": body.rut, "rubro": body.rubro,
...
@router.put("/{prov_id}")
def actualizar_proveedor(prov_id: int, body: ProveedorUpdate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
def eliminar_proveedor(prov_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
def ordenes_proveedor(prov_id: int, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    tenant_id = current_user["tenant_id"]
    _ensure_tables(db)
...
def crear_orden(body: OTCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    tenant_id = current_user["tenant_id"]
    _ensure_tables(db)
    ...
    ), {"tid": tenant_id, "pvid": body.proveedor_id, "tit": body.titulo,
...
def cambiar_estado_ot(ot_id: int, body: OTEstado, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
